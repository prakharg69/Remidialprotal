...
<body class="h-full text-slate-100 bg-slate-955 antialiased overflow-hidden select-none"
      x-data="{
          started: false,
    ...
    <div x-show="!started" class="fixed inset-0 z-50 bg-slate-950 flex items-center justify-center p-6">
        ...
        <div class="relative bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-xl shadow-2xl p-8 sm:p-10 text-center space-y-8 animate-fade-in">
            ...
            <div class="bg-slate-955/60 border border-slate-850 rounded-2xl p-5 text-left text-xxs font-medium text-slate-400 space-y-3">
                <h4 class="text-xs font-black text-slate-300 uppercase tracking-wide">Assessment Security Directives:</h4>
    ...
    <div x-show="started && !fullscreenActive" x-cloak class="fixed inset-0 z-40 bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-6">
        <div class="bg-slate-900 border border-rose-500/30 rounded-3xl w-full max-w-md shadow-2xl p-8 text-center space-y-6 animate-fade-in">
            <div class="h-16 w-16 rounded-full bg-rose-500/10 flex items-center justify-center mx-auto text-rose-500 border border-rose-500/20 shadow-inner">
            ...
            <div class="space-y-2">
                <h3 class="text-lg font-black text-rose-500 uppercase tracking-wide">⚠️ Security Alert: Screen Escaped</h3>
                ...
                <p class="text-xxs font-bold text-slate-400 bg-rose-500/10 border border-rose-500/20 p-2.5 rounded-xl mt-3">
    ...
    <div x-show="started" x-cloak class="h-full flex flex-col justify-between">
        ...
        <header class="bg-slate-900 border-b border-slate-800 px-6 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4 bg-slate-950/60 border border-slate-800 rounded-xl px-4 py-2 shadow-inner">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xxxxs uppercase tracking-wider text-slate-500 font-extrabold">Time Remaining:</span>
        ...
        <main class="flex-1 overflow-hidden flex flex-col md:flex-row">
            ...
            <aside class="w-full md:w-64 bg-slate-900/50 border-r border-slate-800 p-6 flex flex-col gap-6 overflow-y-auto">
                <div>
                    <h4 class="text-xxxxs uppercase tracking-widest text-slate-500 font-black">Assessment Navigation</h4>
                    <p class="text-xxxxs text-slate-500 mt-0.5">Click number to jump to question.</p>
                </div>
                    ...
                    @foreach($quiz->questions as $idx => $q)
                        <button type="button" 
                                ...
                                :class="{
                                    ...
                                    'border-slate-800 bg-slate-950/40 text-slate-500 hover:border-slate-700': currentQuestionIndex !== {{ $idx }} && answeredQuestions['{{ $q['id'] }}'] === undefined
                                }"
                ...
                <!-- Navigation Legend -->
                <div class="mt-auto space-y-2 border-t border-slate-850 pt-4 text-xxxxs font-bold uppercase tracking-wider text-slate-500">
                    <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="h-2 w-2 rounded bg-slate-800"></span> Unanswered
                    </div>
                </div>
            </aside>
            ...
            <section class="flex-1 overflow-y-auto p-6 sm:p-10 flex flex-col justify-center max-w-4xl mx-auto w-full">
                ...
                <form id="submitQuizForm" action="{{ route('student.quizzes.submit', $attempt->id) }}" method="POST">
                        ...
                        <div x-show="currentQuestionIndex === {{ $idx }}" 
                            ...
                            <div class="grid grid-cols-1 gap-4 pt-2">
                                @foreach($q['options'] as $optIndex => $optionText)
                                    <label class="relative flex items-center gap-4 rounded-xl border border-slate-850 bg-slate-900/40 p-5 hover:bg-slate-900 hover:border-slate-700 cursor-pointer transition-all shadow-sm select-none"
                                           :class="{
                                               'border-indigo-500 bg-slate-900/80 shadow-md shadow-indigo-950/20': answeredQuestions['{{ $q['id'] }}'] === {{ $optIndex }},
                                               'border-slate-850 bg-slate-900/40': answeredQuestions['{{ $q['id'] }}'] !== {{ $optIndex }}
                                           }">
                                        <input type="radio" 
                                               name="answers[{{ $q['id'] }}]" 
                                               value="{{ $optIndex }}" 
                                               ...
                                               class="h-4 w-4 text-indigo-600 border-slate-800 bg-slate-950 focus:ring-indigo-500">
                                        
                                        <span class="h-6 w-6 rounded bg-slate-950 border border-slate-850 flex items-center justify-center font-bold text-xxs uppercase text-slate-500">
                                            {{ chr(65 + $optIndex) }}
                                        </span>
                                        <span class="text-xs sm:text-sm font-semibold text-slate-300 leading-relaxed">{{ $optionText }}</span>
                                    </label>
                                @endforeach
                            </div>
                    ...
                    <div class="flex items-center justify-between border-t border-slate-850 pt-8 mt-8">
                        <button type="button" 
                                @click="if(currentQuestionIndex > 0) currentQuestionIndex--"
                                :disabled="currentQuestionIndex === 0"
                                class="inline-flex items-center gap-1.5 rounded-xl border border-slate-800 bg-slate-900 hover:bg-slate-850 text-slate-350 font-bold text-xxs py-3 px-6 shadow transition-all uppercase tracking-wider disabled:opacity-30 disabled:cursor-not-allowed">
                        ...
                        <div class="flex-shrink-0">
                            ...
                            <button type="button" 
                                    ...
                                    class="inline-flex items-center gap-1.5 rounded-xl bg-slate-800 hover:bg-slate-750 text-white font-bold text-xxs py-3 px-6 shadow border border-slate-700 transition-all uppercase tracking-wider">
                                Next Question &rarr;
                            </button>
    ...
    <div id="cheatModal" class="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-6 hidden">
        <div class="bg-slate-900 border border-rose-500/30 rounded-3xl w-full max-w-md shadow-2xl p-8 text-center space-y-6 animate-fade-in">
            <div class="h-16 w-16 rounded-full bg-rose-500/10 flex items-center justify-center mx-auto text-rose-500 border border-rose-500/20 shadow-inner">
                <svg class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
            </div>
            ...
            <div class="space-y-2">
                <h3 class="text-xl font-black text-rose-500 uppercase tracking-wide">⚠️ Cheating Policy Violation Warning</h3>
                <p class="text-xs text-slate-355 leading-relaxed">
                ...
                <p class="text-xs font-bold text-slate-300 mt-2 bg-rose-500/10 border border-rose-500/20 p-2.5 rounded-xl">
